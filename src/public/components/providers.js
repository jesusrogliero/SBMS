...
let costs = Vue.component('costs', {
  ...
  computed: {
    formTitle () {
      return this.editedIndex === -1 ? 'Nuevo Provedor' : 'Actualizar un Provedor';
    },
  },
  ...
  methods: {
    ...
    editItem: async function(item) {
      ...
      this.editedItem = await execute('show-provider', this.editedIndex);
      this.dialog = true
    },
    ...
    deleteItem: async function(item) {
      ...
      if(this.editedItem.code == 0){
        ...
        this.editedItem = this.cleanForm();
      }
    ...
    deleteItemConfirm: async function() {
      let result = await execute('destroy-provider', this.editedIndex);
    ...
    save: async function() {
      ...
      if (this.editedIndex > -1) {
        result = await execute('update-provider', this.editedItem);
      } else {
        result = await execute('create-provider', this.editedItem);
      }
  ...
  <template v-slot:top>
    <v-toolbar flat >
      <v-toolbar-title>Provedores</v-toolbar-title>
      <v-divider
        class="mx-4"
        inset
        vertical
